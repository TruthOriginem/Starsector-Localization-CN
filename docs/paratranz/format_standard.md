...
> 请注意，内核 jar 文件的翻译规则与其他部分有所不同，请参考文末的 [内核 .jar 文件汉化规则](#内核-jar-文件汉化规则)
...
- 人名
- 恒星/星系名、行星名
  - 部分代码依赖于这些名字，翻译将导致游戏崩溃
  - 如果是恒星名与设施名的组合，则只翻译设施部分
    - 例 `Ragnar·Complex` 译为 `Ragnar·综合设施`
...
- 连续的标点中间不空格
    ...
    - 但括号与引号之间空格
      - 例：`gaFC_laicaille:"我有非得这么做的理由！"·(虚张声势)`
- **不能连续空格** *（不适用于内核jar文件）*
- 段首不空格 *（不适用于内核jar文件）*
...
- `$heOrShe`会被系统替换成`他`或者`她`。
- `$hisOrHer`、`$himOrHer`、`$himOrHerself`**也会被系统替换成`他`或者`她`**
    - 如果需要翻译成`他(她)的`、`他(她)自己`，请写成`$hisOrHer的`、`$himOrHerself自己`。
...
- **词条中的`%`必须一一对应，不可增减。** 这是标记插入数字或者单一高亮单词的占位符
  - **前后必须有空格**
...
- 不需要强求每个翻译词条都有字，中文表达中不需要的词可以将译文留空
  - 例如部分英文特有的语言结构，如单复数，`a`和`an`，`The`等根据情况可以留空
- 如果遇到格式不确定的情况，请将词条标为`有疑问`并咨询`灰色渡鸦`或`jn_xyp`
