    ...
    for root, dirs, files in os.walk(dir1):
        for file in files:
            ...
            if os.path.isfile(original_old_file) and os.path.isfile(original_new_file):
                if filecmp.cmp(original_old_file, original_new_file):  # 判断文件内容是否一致
                    if os.path.isfile(old_loc_file):
                        copy_if_identical(old_loc_file, new_loc_file, os.path.join(new_loc_dir, relative_path))  # 如果一致就复制文件
...
# 调用函数
compare_and_copy_dirs(original_old_dir, original_new_dir, localization_old_dir, localization_new_dir)