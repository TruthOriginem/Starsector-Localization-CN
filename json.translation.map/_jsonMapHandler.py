...
class MapItem(object):
        ...
        if os.path.isdir(self.source_path):
            for root, sub_folders, filenames in os.walk(self.source_path):
                for filename in filenames:
                    ...
                    print(
                        '\r已搜索到{}个条目(搜索文件总和:{})，已检索到{}个独特条目...'.format(
    ...
    def updateFile(
        self, file_path, new_lines, item_map_json, item_map_values, no_match_item_list
    ):
        line_changed = False
        key_times = 0
        line_count = 0
                        ...
                        if value:
                            ...
                            if item_map_json.__contains__(key):
                                if item_map_json[key].lower() != key:
                                    ...
                                    line = (
                                        line[: display_match.start(1)]
                                        + escaped_value
                                        + line[display_match.end(1) :]
                                    )
                                    line_changed = True
    ...
    def updateTargetItems(self):
        ...
        if os.path.isdir(self.target_path):
            ...
            new_lines = list()
            (line_changed, key_times) = self.updateFile(
                file_path, new_lines, item_map_json, item_map_values, no_match_item_list
            )
            processed_times += key_times
            if line_changed:
                processed_files += 1
...
with open(json_map_collection_path, 'r', encoding='utf-8') as f:
    ...
    for row in dr:
        # print(row)
        items.append(
            MapItem(row['prefix'], row['ext'], row['key'], row['path'], row['mapKey'])
        )
