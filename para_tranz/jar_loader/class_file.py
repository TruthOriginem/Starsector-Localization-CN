...
class JavaClassFile:
    ...
    def get_utf8_constant_pairs(
        ...
        translated_constant_index_constants = {
            c.constant_index: c
        ...
        for original_constant in original_constants:
            ...
            added_strings.add(
                ...
                if not ORIGINAL_TEXT_MATCH_IGNORE_WHITESPACE_CHARS
    ...
    def _get_original_string_to_const_pairs_mapping(
        self,
    ) -> Dict[str, List[Tuple[Utf8Constant, Utf8Constant]]]:
        """
        ...
        for original, translation in self.get_utf8_constant_pairs():
            if original.string not in original_string_to_const_pairs:
                original_string_to_const_pairs[original.string] = [
                    (original, translation)
                ]
            else:
                original_string_to_const_pairs[original.string].append(
                    (original, translation)
                )
    ...
    def update_strings(self, strings: List[String]) -> int:
        ...
        :return: 更新成功的词条数量
        """
        original_string_to_const_pairs = (
            self._get_original_string_to_const_pairs_mapping()
        )
