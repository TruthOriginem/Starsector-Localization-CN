...
`update_strings` 为单遍处理，遍历顺序与 `text_paths` 顺序一致。若同一 Object 的同一层级同时配置了 `.$key`（重命名 key）和 `.*`/具名字段路径（替换 value），则：

1. key 重命名先执行，将原始 key 改为译文 key；
2. value 替换后续尝试按原始 key 查找，找不到 → 静默失败。

**结论**：对同一 Object 同一层级，不要同时在 `text_paths` 中配置 `.$key` 和值路径。

如需支持此场景，可将 `update_strings` 改为两遍：先执行所有 value 替换，再执行所有 key 重命名。当前翻译文件列表中无此场景，故暂不实现。
