...
from para_tranz.util import relative_path, String, DataFile, contains_chinese, replace_weird_chars, make_logger


class CsvFile(DataFile):
    logger = make_logger(f'csv_loader.py')
    ...
    @classmethod
    def load_files(cls) -> List['CsvFile']:
        """
        ...
        """
        cls.logger.info('开始读取游戏原文与译文数据')
        with open(MAP_PATH, encoding='utf-8') as f:
            d = json.load(f)
        files = [cls(**mapping) for mapping in d if
                 mapping['path'].endswith('.csv') or mapping.get('type', '') == 'csv']
        cls.logger.info('游戏原文与译文数据读取完成')
        return files
