...
std::vector<OutputSpec> makeSpecs() {
    ...
    struct {
        ...
        // 中西文字号按实机可读性分别校准，不从 infoSize 推导；上飘取接近行高 10%
        // 的整数值。字体仍沿用原版布局度量，避免字号调校改变 UI 行距与基线。
        {"victor10", 10.0, 900, 1, -10, 10, 9, 11, 0, 0.17, 0, 1, 1, true, true},
        {"victor14", 11.5, 800, 1, -14, 13, 11, 14, 0, 0.13, 0, 1, 1, true, true},
        // victor16 的中文字号取 17 而非 16：锐字 advance/em≈0.963，round(16×0.963)=15
        // 会让汉字排版窄 1px。17 → round=16，与 Zpix 版逐字同宽。
