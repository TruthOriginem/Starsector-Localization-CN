...
class ArgumentTest(unittest.TestCase):
    ...
    def test_can_select_each_package_type(self) -> None:
        ...
        self.assertEqual(make_exe.parse_args(['--package', 'translation']).package,
                         'translation')
...
class PackageSelectionTest(unittest.TestCase):
    def test_translation_only_does_not_access_original_game(self) -> None:
        with mock.patch.dict(os.environ, {'ORIGINAL_GAME_FOLDER': 'Z:/does-not-exist'}), \
                mock.patch.object(make_exe, 'generate_design_type_fragment'), \
                mock.patch.object(make_exe, 'find_iscc', return_value=Path('ISCC.exe')), \
                mock.patch.object(
                    make_exe, 'load_package_metadata', return_value=METADATA
                ), \
        ...
        self.assertEqual(run_iscc.call_args.args[1].name,
                         'ss_translation_pack_installer.iss')
